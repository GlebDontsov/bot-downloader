...
class DownloadHistory(Model):
    ...
    user = fields.ForeignKeyField(
        "models.User", related_name="download_history", description="Пользователь"
    )
    video = fields.ForeignKeyField(
        "models.Video", related_name="download_history", description="Видео"
    )

    # Статус и детали
    status = fields.CharEnumField(
        DownloadStatus, default=DownloadStatus.PENDING, description="Статус скачивания"
    )
    quality = fields.CharField(
        max_length=50, null=True, description="Выбранное качество"
    )
    format_type = fields.CharField(
        max_length=20, default="mp4", description="Тип формата"
    )
    file_size = fields.BigIntField(null=True, description="Размер файла в байтах")
    download_speed = fields.FloatField(
        null=True, description="Скорость скачивания в Мб/с"
    )

    # Пути и файлы
    file_path = fields.TextField(null=True, description="Путь к скачанному файлу")
    telegram_file_id = fields.CharField(
        max_length=255, null=True, description="ID файла в Telegram"
    )
