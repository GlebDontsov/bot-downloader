...
class User(Model):
    ...
    telegram_id = fields.BigIntField(
        unique=True, description="Telegram ID пользователя"
    )
    ...
    is_admin = fields.BooleanField(
        default=False, description="Является ли администратором"
    )
    is_blocked = fields.BooleanField(
        default=False, description="Заблокирован ли пользователь"
    )
    language_code = fields.CharField(max_length=10, null=True, description="Код языка")

    # Статистика
    total_downloads = fields.IntField(
        default=0, description="Общее количество скачиваний"
    )
    total_download_size = fields.BigIntField(
        default=0, description="Общий размер скачанных файлов"
    )

    # Даты
    created_at = fields.DatetimeField(auto_now_add=True, description="Дата регистрации")
    updated_at = fields.DatetimeField(
        auto_now=True, description="Дата последнего обновления"
    )
